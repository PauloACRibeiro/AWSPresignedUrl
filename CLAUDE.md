...
- When the umbrella workspace root is available, follow the root [`CLAUDE.md`](/Users/paulo.cunha.ribeiro/Documents/Workspace/CLAUDE.md) and read shared workspace knowledge directly from `knowledge/...`.
- Do not assume the umbrella workspace root is mounted in cloud tasks.
- If the workspace root is unavailable in a cloud task, use repo-local docs plus the official remote authorities named in the root routing rules. Do not use a hosted workspace MCP.
